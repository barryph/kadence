      ...
      >
        {resolved ? (
          <img
            src={resolved}
            alt={alt}
            // `fill`, deliberately: captures are taken at this frame's inner
            // screen ratio (see DECKS in tools/generate.mjs), so the image lands
            // edge to edge with nothing to hide. `contain` would leave the
            // frame's background showing as a bar; `cover` would silently drop
