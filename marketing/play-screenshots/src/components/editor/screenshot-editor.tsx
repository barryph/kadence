...
export function ScreenshotEditor() {
   ...
   *
   * The deck is also rendered without a human at the keyboard (Node drives the
   * browser over CDP). Exposing the same handler the toolbar button uses keeps
   * one export implementation, so an automated run and a manual click cannot
   * drift apart.
   */
  React.useEffect(() => {
    (window as unknown as { __dshExport?: () => Promise<void> }).__dshExport = exportAll;
  });
  ...
  /**
   * Deep links for review and automation: `?device=android-10&slide=3` opens
   * that deck and screen directly, which is how the deck is verified at export
   * size without clicking through the toolbar.
   */
  React.useEffect(() => {
    ...
    if (device && device in state.slidesByDevice) next.device = device as Device;
    if (connected === "0" || connected === "1") {
      next.connectedCanvas = connected === "1";
    }
    if (Object.keys(next).length > 0) {
      setState((prev) => ({ ...prev, ...next }));
    }
    if (Number.isFinite(slide) && slide >= 0) {
      const deck = state.slidesByDevice[(next.device as Device) ?? state.device] || [];
      if (deck[slide]) setActiveSlideId(deck[slide].id);
    }
    // Only on first hydration; the URL is a one-shot entry point.
