...
/**
 * The headline, with the final line carrying the brand accent.
 *
...
function Headline({
  ...
  const fg = inverted ? theme.fgAlt : theme.fg;
  const text = pickText(slide.headline, locale);
            ...
            <span
              style={
                isAccent
                  ? {
                    color: theme.accent,
                    textShadow: `0 0 ${unit * 0.045}px ${hexAlpha(theme.accent, 0.55)}`,
                  }
                  : undefined
              }
            >
...
/** The theme whose backgrounds get the layered bloom treatment. */
const DEEP_SPACE_THEME_ID = "deep-space";

function isDeepSpace(theme: Theme) {
  return theme.id === DEEP_SPACE_THEME_ID;
}

function backgroundFor(theme: Theme, inverted?: boolean) {
  if (isDeepSpace(theme)) {
    // Near-black navy in the body, electric blue climbing in at the bottom
    // corners, with a cyan highlight above it - the app's own canvas gradient.
    return inverted
      ? `linear-gradient(168deg, #02030A 0%, #050711 45%, #071229 80%, #0A1E3C 100%)`
      : `linear-gradient(168deg, ${DEEP_SPACE.canvas} 0%, ${DEEP_SPACE.canvasMid} 42%, #0A1730 74%, #102A52 100%)`;
  }
  if (inverted) {
...
/**
 * Electric blue -> cyan bloom, in two corners. Pure gradient layers, so they
 * survive the PNG snapshot unchanged (unlike CSS filters on large nodes).
 */
function deepSpaceBloom(cW: number, cH: number, inverted?: boolean) {
  const strength = inverted ? 0.5 : 0.78;
  return [
    `radial-gradient(58% 34% at 12% 106%, ${hexAlpha(DEEP_SPACE.accentGlow, strength)} 0%, ${hexAlpha(DEEP_SPACE.accentBright, strength * 0.45)} 34%, transparent 72%)`,
    `radial-gradient(52% 30% at 96% 100%, ${hexAlpha(DEEP_SPACE.cyan, strength * 0.8)} 0%, ${hexAlpha(DEEP_SPACE.accentGlow, strength * 0.4)} 40%, transparent 74%)`,
    `radial-gradient(70% 42% at 50% 122%, ${hexAlpha(DEEP_SPACE.accentBright, strength * 0.32)} 0%, transparent 70%)`,
    `radial-gradient(46% 26% at 78% -6%, ${hexAlpha(DEEP_SPACE.navy, 0.85)} 0%, transparent 70%)`,
  ].join(", ");
}

/**
 * Fine film grain. Without it, a near-black gradient bands badly on the
 * large-format exports. Rendered as an inline SVG so it needs no asset file.
 */
function grainLayer(opacity: number) {
  const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='160' height='160'><filter id='n'><feTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='2' stitchTiles='stitch'/><feColorMatrix type='saturate' values='0'/></filter><rect width='160' height='160' filter='url(%23n)' opacity='${opacity}'/></svg>`;
...
function SlideBackground({
  ...
  const deepSpace = isDeepSpace(theme);
  return (
    <div
      style={{
        position: "absolute",
        ...
        <>
          <div
            style={{
              position: "absolute",
              inset: 0,
              background: deepSpaceBloom(cW, cH, inverted),
              pointerEvents: "none",
            }}
          />
          <div
            style={{
              position: "absolute",
              inset: 0,
              backgroundImage: grainLayer(0.05),
              opacity: 0.55,
  ...
  return (
    <div
      ...
      style={{
        ...
        inset: 0,
        pointerEvents: "none",
        outline: `${active ? Math.max(4, cW * 0.003) : Math.max(2, cW * 0.0015)}px solid ${active ? "rgba(91, 124, 250, 0.95)" : "rgba(15, 23, 42, 0.22)"
          }`,
    ...
    <div
      style={{
        ...
        background: deepSpace
          ? backgroundFor(theme)
          : `linear-gradient(135deg, ${theme.bgAlt} 0%, ${shade(theme.bgAlt, -10)} 50%, ${theme.accent} 200%)`,
        display: "flex",
        alignItems: "center",
        padding: `0 ${cW * 0.06}px`,
        color: deepSpace ? theme.fg : theme.fgAlt,
      }}
    >
      {deepSpace ? (
        <>
          <div
            style={{
              position: "absolute",
              inset: 0,
              background: deepSpaceBloom(cW, cH),
              pointerEvents: "none",
            }}
          />
          {/* A slice of the product itself, bleeding off the right edge. */}
          <div
            style={{
              position: "absolute",
              right: -cW * 0.03,
              ...
              overflow: "hidden",
              opacity: 0.95,
              boxShadow: `0 0 ${cW * 0.06}px ${hexAlpha(DEEP_SPACE.accentGlow, 0.55)}`,
              pointerEvents: "none",
            }}
          >
            {img("/screenshots/android/phone/en/04-goals-list.png") ? (
              <img
                src={img("/screenshots/android/phone/en/04-goals-list.png")}
                alt=""
        ...
        {appIcon && img(appIcon) ? (
          <img
            ...
            style={{
              ...
              boxShadow: deepSpace
                ? `0 0 ${cW * 0.05}px ${hexAlpha(DEEP_SPACE.accentGlow, 0.6)}, 0 4px 16px rgba(0,0,0,0.4)`
          ...
          <div
            ...
            style={{
              ...
              color: deepSpace ? theme.bg : theme.fgAlt,
              fontWeight: 800,
              fontSize: cW * 0.07,
        ...
        <div>
          <div
            style={{
              ...
              fontWeight: 800,
              lineHeight: 1.05,
              letterSpacing: deepSpace ? -cW * 0.0012 : undefined,
            }}
          >
            {appName || "App"}
          </div>
          {deepSpace ? (
            <div
              style={{
                ...
                marginTop: cW * 0.012,
                borderRadius: 999,
                background: `linear-gradient(90deg, ${DEEP_SPACE.accentBright}, ${DEEP_SPACE.cyan})`,
                boxShadow: `0 0 ${cW * 0.012}px ${hexAlpha(DEEP_SPACE.cyan, 0.8)}`,
              }}
            />
          ) : null}
          <Headline
